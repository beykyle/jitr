...
class Rmatrix_free:
    ...
    def free_matrix(
        ...
        free_matrix = self.kinetic_matrix(a, l, hbar_2mu) - self.energy_matrix(a, l, E)
    ...
    def batched_free_matrix(self,
        ...
        r"""        Precompute the free matrix (kinetic + energy) for each shape in the
        batched dictionary of quantum numbers, which only depends on the
        channel orbital angular momenta l and dimensionless channel radius a
        @parameters:
            a : channel radius (r_f)
            l : dictionary of orbital angular momenta for each shape
            E : dictionary of channel energies for each shape
            hbar_2mu : reduced mass of the system. currently restricted to systems
                       in the same mass partition
            coupled : whether to return the full matrix or just the block
                      diagonal elements  (elements off of the channel diagonal
        ...
        for key in keys:
            ...
            E_batch = E_dict[key]
            free_matrix_arr = []
            for l, E in zip(l_batch, E_batch):
                free_matrix_arr.append(
                    self.free_matrix(a, l, E, hbar_2mu, coupled=coupled)
                )
            free_matrices_batch[key] = np.array(free_matrix_arr)
        
        return free_matrices_batch
